...
function palette() {
  return {
    ink: cssVar('--ink', '#21353d'),
    paper: cssVar('--paper', '#faf3e6'),
    slate: cssVar('--slate', '#5484a4'),
    steel: cssVar('--steel', '#acc0d3'),
    rose: cssVar('--rose', '#d396a6'),
    apricot: cssVar('--apricot', '#f6c992'),
    coral: cssVar('--coral', '#fa6e81'),
    teal: cssVar('--teal', '#09a1a1'),
    deepSea: cssVar('--deep-sea', '#30525c'),
  };
}
    ...
    draw(ctx, w, h, _t, dt) {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < COLS; x++) {
          const i = y * COLS + x;
          if (!grid[i]) continue;
          ctx.fillStyle = age[i] > 6 ? pal.slate : age[i] > 2 ? pal.teal : pal.coral;
...
function terminalScene(pal: Pal, lines: string[], title: string): Scene {
  ...
  return {
    draw(ctx, w, h, t) {
      ...
      // Title bar
      ctx.fillStyle = 'rgba(250,243,230,0.08)';
      ...
      for (const line of lines) {
        ...
        ctx.fillStyle = isPrompt ? pal.coral : line.includes('ok') || line.includes('✓') ? pal.teal : pal.paper;
        ...
        if (budget < 0 || (budget === 0 && phase * CPS < total)) {
          ...
          if (Math.floor(t * 2.6) % 2 === 0) {
            ...
            ctx.fillRect(x0 + cw + size * 0.15, y - size * 0.85, size * 0.55, size);
          }
        }
        y += lh;
        if (y > h - lh * 0.4) break;
      ...
      if (phase * CPS >= total && Math.floor(t * 2.6) % 2 === 0) {
        ctx.fillStyle = pal.coral;
        ctx.fillRect(x0, Math.min(y, h - lh) - size * 0.85, size * 0.55, size);
      }
    },
  };
}
...
function chessScene(pal: Pal): Scene {
  ...
  const sq = (file: string, rank: string) => (8 - Number(rank)) * 8 + (file.charCodeAt(0) - 97);
  const MOVES = ['e2e4', 'e7e5', 'g1f3', 'b8c6', 'f1c4', 'f8c5', 'c2c3', 'g8f6', 'd2d4', 'e5d4', 'c3d4', 'c5b4'];
  const MOVE_T = 0.55;
  const PAUSE_T = 0.75;
  const RESET_T = 1.4;
  ...
  return {
    draw(ctx, w, h, t) {
      ...
      for (let r = 0; r < 8; r++) {
        for (let f = 0; f < 8; f++) {
          ctx.fillStyle = (r + f) % 2 === 0 ? pal.apricot : pal.slate;
      ...
      if (done < MOVES.length) {
        ...
        if (local < MOVE_T) {
          ...
          flying = { glyph: board[from]!, x: fx + (tx - fx) * e, y: fy + (ty - fy) * e };
          flightTo = to;
          board[from] = null;
          // highlight target
          ctx.fillStyle = 'rgba(250,110,129,0.4)';
          ctx.fillRect(ox + (to % 8) * cell, oy + Math.floor(to / 8) * cell, cell, cell);
        }
      }
      ...
      board.forEach((glyph, i) => {
        ...
        drawPiece(glyph, ox + (i % 8) * cell + cell / 2, oy + Math.floor(i / 8) * cell + cell / 2);
      });
      ...
      if (phase > cycle - RESET_T) {
        ...
        ctx.fillStyle = `rgba(33,53,61,${Math.sin(p * Math.PI) * 0.85})`;
        ctx.fillRect(0, 0, w, h);
      }
      ctx.textAlign = 'left';
...
function keysScene(pal: Pal): Scene {
  ...
  return {
    draw(ctx, w, h, t) {
      ...
      const ky = h * 0.14;
      const drawKey = (x: number, letter: 'A' | 'D', pressed: boolean) => {
        const dy = pressed ? kw * 0.07 : 0;
        ctx.fillStyle = 'rgba(250,243,230,0.12)';
        roundRect(ctx, x, ky + kw * 0.09, kw, kw, kw * 0.16);
        ctx.fill();
        ctx.fillStyle = pressed ? pal.coral : pal.paper;
      ...
      const lanes: Array<{ label: string; test: (q: number) => boolean; color: string }> = [
        { label: 'raw A', test: aHeld, color: pal.slate },
        { label: 'raw D', test: dHeld, color: pal.rose },
        {
          label: 'resolved',
      ...
      lanes.forEach((lane, li) => {
        ...
        for (let i = 0; i < N; i++) {
          ...
          if (lane.label === 'resolved') color = dHeld(q) ? pal.coral : pal.teal;
          ctx.fillStyle = color;
      ...
      ctx.fillStyle = out ? pal.coral : pal.steel;
      ctx.fillText(out ? `→ output: ${out}  (last input wins)` : '→ output: —', laneX, h * 0.53);
    },
  };
}
...
function waveformScene(pal: Pal): Scene {
  ...
  const colors: Array<keyof Pal> = ['coral', 'teal', 'rose'];
  ...
  return {
    draw(ctx, w, h, t, dt) {
      ...
      dots.forEach((d) => {
        const c = centers[d.c];
        d.x += (c.x - d.x) * dt * 0.22 + Math.sin(t * 0.9 + d.seed) * 0.0007;
        d.y += (c.y - d.y) * dt * 0.22 + Math.cos(t * 0.8 + d.seed) * 0.0007;
        ctx.fillStyle = pal[colors[d.c]];
        ctx.globalAlpha = 0.85;
        ctx.beginPath();
        ctx.arc(d.x * w, d.y * h + h * 0.06, Math.max(1.6, w * 0.004), 0, Math.PI * 2);
        ctx.fill();
      });
      ctx.globalAlpha = 1;
      ...
      for (let i = 0; i < N; i++) {
        ...
        ctx.fillStyle = hot ? pal.coral : pal.slate;
...
function mathScene(pal: Pal): Scene {
  return {
    draw(ctx, w, h, t) {
      ...
      ctx.strokeStyle = 'rgba(84,132,164,0.4)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ...
      ctx.fillStyle = pal.coral;
      ctx.beginPath();
      ctx.arc(cx + r * Math.cos(th), cy + r * Math.sin(th), Math.max(3, R * 0.03), 0, Math.PI * 2);
      ctx.fill();
...
function clockScene(pal: Pal): Scene {
  const TOKENS = 8;
  const tokenColors: Array<keyof Pal> = ['coral', 'teal', 'rose', 'steel', 'apricot', 'slate', 'coral', 'teal'];
  return {
    draw(ctx, w, h, t) {
      ctx.fillStyle = pal.ink;
      ctx.fillRect(0, 0, w, h);
      ...
      for (let i = 0; i < TOKENS; i++) {
        ...
        const rad = R * (isActive ? 0.12 + Math.sin(t * 6) * 0.012 : 0.09);
        if (isActive) {
          ctx.strokeStyle = pal.coral;
          ctx.setLineDash([4, 5]);
          ctx.beginPath();
...
function browserScene(pal: Pal): Scene {
  return {
    draw(ctx, w, h, t) {
      ...
      ctx.fill();
      ctx.fillStyle = pal.apricot;
      roundRect(ctx, bx, by, bw, bh * 0.11, bw * 0.02);
      ctx.fill();
      [pal.teal, pal.coral, pal.steel].forEach((c, i) => {
        ctx.fillStyle = c;
        ctx.beginPath();
        ctx.arc(bx + bw * 0.035 + i * bw * 0.035, by + bh * 0.055, bw * 0.011, 0, Math.PI * 2);
        ctx.fill();
      });
      ...
      const blocks: Array<[number, number, number, number, string]> = [
        [0.06, 0.18, 0.5, 0.1, pal.slate], // headline
        [0.06, 0.32, 0.34, 0.06, pal.slate], // sub
        [0.06, 0.46, 0.26, 0.4, pal.rose], // product card
        [0.37, 0.46, 0.26, 0.4, pal.rose],
        [0.68, 0.46, 0.26, 0.4, pal.rose],
        [0.68, 0.18, 0.26, 0.14, pal.coral], // CTA
      ];
      blocks.forEach((b, i) => {
        ...
        roundRect(ctx, bx + x * bw, by + y * bh, ww * bw * grow, hh * bh, 4);
        ctx.fill();
        ctx.globalAlpha = 1;
      });
...
function formScene(pal: Pal): Scene {
  ...
  return {
    draw(ctx, w, h, t) {
      ...
      ctx.fillStyle = pal.paper;
      ctx.fillText('PeopleSoft — weekly timesheet', sx + sw * 0.04, sy + headH * 0.64);
      ...
      for (let r = 0; r < ROWS; r++) {
        ...
        const rowP = Math.min(1, Math.max(0, (phase - rowStart) / (ROW_T * 0.82)));
        const activeRow = phase >= rowStart && phase < rowStart + ROW_T;

        if (activeRow) {
          ctx.fillStyle = 'rgba(250,110,129,0.28)';
          ctx.fillRect(sx, y, sw, rowH);
        }
        ctx.strokeStyle = 'rgba(33,53,61,0.15)';
        ctx.beginPath();
        ...
        for (let c = 0; c < CELLS; c++) {
          ...
          for (let s = 0; s <= 12 * cellP; s++) {
            ...
            const yy = y + rowH * 0.55 + Math.sin(s * 1.7 + r) * rowH * 0.08;
...
export function mountPreview(canvas: HTMLCanvasElement, key: string): PreviewController {
